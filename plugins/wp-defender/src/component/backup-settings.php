...
class Backup_Settings extends Component {
	 ...
	 * @return void
	 */
	private function create_basic_config(): void {
		$configs = [];
				...
				if ( 'security_tweaks' === $module ) {
					...
					if ( ! empty( $module_data ) && isset( $module_data['notification'] ) ) {
						...
						if ( isset( $module_data['notification_repeat'] ) ) {
							...
							if ( is_bool( $module_data['notification_repeat'] ) ) {
								...
								&& in_array( $module_data['notification_repeat'], [ 'daily', 'weekly', 'monthly' ], true )
							) {
								$tweak_notification->configs['reminder'] = $module_data['notification_repeat'];
							} else {
						...
						if ( ! empty( $module_data['subscribers'] ) ) {
							...
							$tweak_notification->in_house_recipients = [];
							$tweak_notification->out_house_recipients = [];
					...
					if ( ! empty( $module_data ) ) {
						...
						if ( isset( $module_data['notification'] ) ) {
							...
							if ( ! empty( $module_data['notification_subscribers'] ) ) {
								// Reset all recipients before.
								$scan_notification->in_house_recipients = [];
								$scan_notification->out_house_recipients = [];
						...
						if ( ! empty( $module_data['report_subscribers'] ) ) {
							// Reset all recipients before.
							$scan_report->in_house_recipients = [];
							$scan_report->out_house_recipients = [];
					...
					if ( ! empty( $module_data ) ) {
						...
						if ( isset( $module_data['notification'] ) ) {
							...
							if ( ! empty( $module_data['notification_subscribers'] ) ) {
								//Reset all recipients before
								$lockout_notification->in_house_recipients = [];
								$lockout_notification->out_house_recipients = [];
								foreach ( $module_data['notification_subscribers'] as $key => $subscribers ) {
						...
						if ( $is_pro && isset( $module_data['report'] ) ) {
							...
							if ( ! empty( $module_data['report_subscribers'] ) ) {
								// Reset all recipients before.
								$lockout_report->in_house_recipients = [];
								$lockout_report->out_house_recipients = [];
								foreach ( $module_data['report_subscribers'] as $key => $subscribers ) {
					...
					if ( ! empty( $module_data ) ) {
						...
						if ( ! empty( $module_data['subscribers'] ) ) {
							// Reset all recipients before.
							$audit_report->in_house_recipients = [];
							$audit_report->out_house_recipients = [];
							foreach ( $module_data['subscribers'] as $key => $subscribers ) {
	...
	/**
	 * @param string $module
	 * @param bool $all_firewall_lockouts
	 *
	 * @return mixed
	 */
	 ...
	 * @return array
	 */
	private function format_blocklist_monitor_strings( $config, bool $is_pro ): array {
		// If Blocklist Monitor is enable.
		if ( isset( $config['status'] ) && '1' === (string) $config['status'] ) {
			if ( $is_pro ) {
				$monitor = [ __( 'Active', 'wpdef' ) ];
			} else {
				$monitor = [
					sprintf(
